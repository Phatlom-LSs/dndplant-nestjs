...
type Dept = { idx:number; name:string; area:number; fixed:boolean };
...
const SIDE_DIRS = [ {dx:1,dy:0,w:1}, {dx:-1,dy:0,w:1}, {dx:0,dy:1,w:1}, {dx:0,dy:-1,w:1} ];
const CORN_DIRS = [ {dx:1,dy:1,w:0.5}, {dx:1,dy:-1,w:0.5}, {dx:-1,dy:1,w:0.5}, {dx:-1,dy:-1,w:0.5} ];
...
export class AldepService {
  ...
  private fits(owner:number[][], W:number, H:number, x:number, y:number, w:number, h:number){
    if (x<0 || y<0 || x+w>W || y+h>H) return false;
    for (let yy=y; yy<y+h; yy++) for (let xx=x; xx<x+w; xx++)
      if (owner[yy][xx] !== -1) return false;
    return true;
  }

  private mark(owner:number[][], x:number, y:number, w:number, h:number, idx:number){
    for (let yy=y; yy<y+h; yy++) for (let xx=x; xx<x+w; xx++) owner[yy][xx] = idx;
  }
  ...
  private buildOrder(
    depts:Dept[], letters:string[][], lowerBound:ClosenessLetter,
    R:()=>number
  ){
    ...
    order.push(first); placed.add(first);
    ...
    while (placed.size < n) {
      ...
      for (const i of idxs) {
        ...
        placed.forEach(j=>{
          ...
          const r1 = this.letterRank(lij), r2 = this.letterRank(lji);
      ...
      if (cand.length) {
        ...
        for (const i of cand){
          ...
          placed.forEach(j=>{
            ...
            const rank = Math.min(this.letterRank(lij as any), this.letterRank(lji as any));
        ...
        order.push(best); placed.add(best);
      } else {
  ...
  private placeBySweep(
    order:number[], depts:Dept[], W:number, H:number,
    allowSplitting:boolean, maxFragments:number, stripWidth:number
  ){
      ...
      for (const {w,h} of fp) {
        for (const {x,y} of starts) {
          if (this.fits(owner, W, H, x, y, w, h)) {
            ...
            placements.push({ name:d.name, idx:i, x, y, width:w, height:h });
            remain = 0; doneWhole = true; break;
          }
        }
        if (doneWhole) break;
      }
        ...
        for (const {x,y} of starts) {
          ...
          for (let a=maxA; a>=1; a--) {
            for (let w=1; w<=a; w++) {
              if (a%w) continue;
              const h = a/w;
              if (this.fits(owner, W, H, x, y, w, h)) { best = {x,y,w,h}; break; }
            }
            if (best) break;
          }
          if (!best) continue;
          this.mark(owner, best.x, best.y, best.w, best.h, i);
          placements.push({ name:d.name, idx:i, x:best.x, y:best.y, width:best.w, height:best.h });
          remain -= best.w*best.h; frag++;
          if (remain<=0 || frag>=Math.max(1, maxFragments)) break outer;
        }
        for (const {x,y} of starts) {
          if (remain<=0) break;
          if (owner[y][x] === -1) { owner[y][x]=i; placements.push({ name:d.name, idx:i, x, y, width:1, height:1 }); remain--; }
        }
      }
  ...
  private scorePR(placements:Rect[], owner:number[][], letters:string[][], Wmap:Weights){
    let total = 0;
    const H = owner.length, W = owner[0]?.length ?? 0;
  ...
  async generate(dto: GenerateAldepDto){
    ...
    const gridW = dto.gridWidth, gridH = dto.gridHeight;
    const lowerBound = (dto.lowerBound ?? 'A') as ClosenessLetter;
    ...
    const depts: Dept[] = (dto.departments||[])
      .filter(d=> (d?.area ?? 0) > 0)
      .map((d, i)=> ({ idx:i, name:d.name, area:Math.floor(d.area), fixed:!!d.fixed }));
    ...
    for (let s=0; s<seeds; s++) {
      ...
      const { owner, placements } = this.placeBySweep(order, depts, gridW, gridH, allowSplitting, maxFragments, stripWidth);
    ...
    const result = {
      mode: 'ALDEP',
      params: { lowerBound, stripWidth, seeds, allowSplitting, maxFragmentsPerDept: maxFragments },
      score: { total: bestScore },
      order: best.order.map((i:number)=> depts[i].name),
      placements: best.placements,
      ownerGrid: best.owner,
      grid: { width: gridW, height: gridH, cellSizeMeters: dto.cellSizeMeters ?? null },
    };
